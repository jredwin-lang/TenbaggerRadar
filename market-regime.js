/* Tenbagger Radar v2.63 market regime / breadth / sector rotation overlay. Facts are calculated only from market.json; no sample data. */
(()=>{'use strict';
...
function render(data){const host=document.querySelector('#pane-macro');if(!host)return;const r=regime(data),sectors=sectorRows(data),watch=watchRotation(data);
 ...
 section.innerHTML=`<div class="section-head"><div><div class="eyebrow">MARKET REGIME · v2.63</div><h2>시장 → 자금 → 산업</h2></div><div class="mr-score"><strong>${num(r.score,0)}</strong><span>/100 · ${esc(r.label)}</span></div></div>
 ...
 host.prepend(section);document.querySelector('.workspace-intro .eyebrow')?.replaceChildren(document.createTextNode('TENBAGGER RADAR v2.63 · MARKET WORKSPACE'));
}
