/* Tenbagger Radar v2.62 market regime / breadth / sector rotation overlay. Facts are calculated only from market.json; no sample data. */
(()=>{'use strict';
...
function regime(data){const nas=data.macro?.find(x=>x.symbol==='^IXIC'),nm=metrics(nas),vix=metrics(data.macro?.find(x=>x.symbol==='^VIX')),dxy=metrics(data.macro?.find(x=>x.symbol==='DX-Y.NYB'));
 ...
 const trend=nm&&[nm.ma20,nm.ma60,nm.ma120].every(Number.isFinite)?[nm.ma20,nm.ma60,nm.ma120].reduce((total,ma)=>total+(nm.p>ma?10:0),0):null;
 ...
 const fear=data.fear?.score,sent=Number.isFinite(fear)?Math.max(0,Math.min(10,fear/10)):null;
...
function render(data){const host=document.querySelector('#pane-macro');if(!host)return;const r=regime(data),sectors=sectorRows(data),watch=watchRotation(data);
 ...
 section.innerHTML=`<div class="section-head"><div><div class="eyebrow">MARKET REGIME · v2.62</div><h2>시장 → 자금 → 산업</h2></div><div class="mr-score"><strong>${num(r.score,0)}</strong><span>/100 · ${esc(r.label)}</span></div></div>
