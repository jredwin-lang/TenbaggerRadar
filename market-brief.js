...
const topics=[
 {test:/\bfed\b|federal reserve|interest.rate|higher.rate|rising.rate|rate.outlook|treasury|bond.yield/i,name:'금리·연준',risk:'금리 또는 채권수익률이 다시 상승하면 성장주의 가치평가 부담이 커질 수 있습니다',check:'금리 관련 제목의 방향을 연준 원문·국채수익률과 대조',source:'https://www.federalreserve.gov/monetarypolicy.htm'},
 ...
 {test:/\boil\b|\bcrude\b|\biran\b|\bhormuz\b|\bwar\b|\btariffs?\b/i,name:'정책·공급 충격',risk:'공급 차질이나 관세가 실제 비용 증가로 이어지는지 확인이 필요합니다',check:'보도된 정책·사건과 실제 시행·공급 차질을 구분',source:null},
...
function selectNews(data){
 ...
 return {recent:recent.length>0,cutoff,items:pool.filter(a=>!/print edition|latest news|news archive|homepage/i.test(a.title)).filter(a=>/wall street|s&p|nasdaq|\bdow\b|\bstocks?\b|federal reserve|\bfed\b|inflation|\bcpi\b|\bpce\b|treasury|bond.yield|dollar|interest.rate|higher.rate|rising.rate|rate.outlook/i.test(a.title)).slice().sort((a,b)=>(priority.indexOf(a.publisher)<0?99:priority.indexOf(a.publisher))-(priority.indexOf(b.publisher)<0?99:priority.indexOf(b.publisher))||Date.parse(b.publishedAt)-Date.parse(a.publishedAt)).filter(a=>{if(seen.has(a.publisher))return false;seen.add(a.publisher);return true}).slice(0,6)};
}
function build(data){
 ...
 {kind:'기사 → Opinion',text:themes.length?kst(themes[0].article.publishedAt)+' 발행 '+themes[0].article.publisher+' 기사의 공개 제목에서 '+themes[0].name+' 이슈가 확인돼, '+'다음 사항을 확인하겠습니다: '+themes[0].check+'.':'현재 수집 구간에서 해석 근거로 사용할 경제 전문지 기사를 확인하지 못해 뉴스 원인은 판단을 보류합니다.',href:themes[0]?.article.url,label:'관련 기사'},
