...
function build(data){
 ...
 {kind:'기사 → Opinion',text:themes.length?kst(themes[0].article.publishedAt)+' 발행 '+themes[0].article.publisher+' 기사의 공개 제목에서 '+themes[0].name+' 이슈가 확인돼, '+themes[0].check+'을 관전 포인트로 삼겠습니다.':'현재 수집 구간에서 해석 근거로 사용할 경제 전문지 기사를 확인하지 못해 뉴스 원인은 판단을 보류합니다.',href:themes[0]?.article.url,label:'관련 기사'},
