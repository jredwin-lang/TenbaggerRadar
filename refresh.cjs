...
if(rows.length<252||rows.some(r=>!r.slice(1).every(Number.isFinite)||r[2]<Math.max(r[1],r[3],r[4])||r[3]>Math.min(r[1],r[2],r[4])||r[5]<0))throw Error(stock.ticker+' invalid history');
if((Date.parse(nyDate)-Date.parse(rows.at(-1)[0]))/86400000>7)throw Error(stock.ticker+' stale source');
Object.assign(stock,{rows,quotePrice:rows.at(-1)[4],quoteVolume:rows.at(-1)[5],quoteTime:raw.meta.regularMarketTime,downloadedAt:now.toISOString(),splits:raw.events?.splits||{},crossQuote:{close:null,volume:null}});
}
...
h=h.replace(/Tenbagger Radar v2\.3 · \d{4}-\d{2}-\d{2}/g,'Tenbagger Radar v2.3 · '+date).replace('정규장 종가 · USD / meta','정규장 종가 · USD / quote.close');
...
h=h.slice(0,start)+`function sourceCard(s){return '<details id="source-'+s.ticker+'"><summary>'+s.ticker+' · 이번 갱신 원자료</summary><p>'+link(s.source,'Yahoo Finance OHLCV')+' · '+link(s.ir,'공식 IR · 다음 실적 재확인 필요')+'</p><p>조회시각 '+s.downloadedAt+' · 완료 일봉 '+s.rows.at(-1)[0]+' · 원자료 행 수 '+s.rows.length+' (계산값). 종가·거래량은 quote.close·quote.volume. 타 공급자 교차 확인 미완료. Investment는 최초 원본 평가 의견.</p><pre style="white-space:pre-wrap;overflow-wrap:anywhere">'+esc(JSON.stringify(s.rows.slice(-5)))+'</pre></details>'}\n`+h.slice(end);
